    ...
    case "show":
        ...
        if (is_array($products)) {
            foreach ($products as $id => $row) {
                $final_order .= "<br>";
                $final_order .= $row['product_name'];

                ?>
                <form  action="mail.php" method="post"  id="cart-checkout" >
                <input name="orders" type="hidden" value="<? $final_order ?>"><br>
                <?php
            }
        } else {
            //!!!!!!SQL FAULT
            echo "SORRY FOR TEMPORARY HITCH";
        }
        ?>
        <input name="order_price" type="hidden" value="<?= sprintf("$%0.2f", $total) ?>"><br>
        ...
        <button type="submit" value="Checkout" onclick="cart.checkout()">Gönder</button>
        </form>
        <!--<form method="post" id="cart-checkout"  onsubmit="return cart.checkout();">
            <label>Name</label>
